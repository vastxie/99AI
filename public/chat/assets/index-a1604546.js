...
`,Qe=Ee({__name:"index",setup(ue){var Y;const te=pe(),c=ge(),B=we(),W=(Y=te.globalConfig)==null?void 0:Y.mindDefaultData,q=X(null),J=new be.Transformer,p=X(!1),{isMobile:j}=he(),F=X(""),N=X(),C=X("");function ne(){C.value=W||ze}fe(()=>{setTimeout(()=>{ne()},1e3)});let G;function V(){const g=new XMLSerializer().serializeToString(N.value),v=new Blob([g],{type:"image/svg+xml;charset=utf-8"});le.saveAs(v,"nineai-mind.svg")}async function K(){const v=`
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
`).forEach(P=>{try{const m=JSON.parse(P);m.userBalance&&te.updateUserBalance(m.userBalance),m.text&&(v+=m.text)}catch{}})}}),p.value=!1})()}catch(g){p.value=!1;const{code:v=500,message:t="好像出错了，请稍后再试！"}=g;if(v===429&&t.includes("balance has been exhausted"))return B.error("当前系统Key余额耗尽、请联系管理员补充！");if(v===500){let e=(g==null?void 0:g.message)??"好像出错了，请稍后再试！";e==="Request failed with status code 401"&&(e="非法操作、请先登录后再进行问答使用！"),B.error(e);return}if(g.code===402){B.error(g.message,{duration:5e3}),c.updateGoodsDialog(!0);return}B.error("出了点小错误、请稍后试试吧！")}finally{p.value=!1}}async function ae(){const g=await Ae(N.value);le.saveAs(g,"markmap.png")}function oe(){C.value=He}const Q=()=>{const{root:g}=J.transform(C.value);G.setData(g),G.fit()};return fe(()=>{G=Pe.create(N.value),Q()}),_e(Q),(g,v)=>(ee(),Z("div",Se,[b("div",Ie,[b("div",Be,[h(j)?(ee(),Z("header",Ue,[b("h2",ke,I(h(T)("mindmap.title")),1)])):de("",!0),b("h4",De,I(h(T)("mindmap.yourNeeds")),1),M(h(ce),{ref_key:"inputRef",ref:q,value:F.value,"onUpdate:value":v[0]||(v[0]=t=>F.value=t),type:"textarea",disabled:p.value,autosize:{minRows:3},placeholder:h(T)("mindmap.inputPlaceholder")},null,8,["value","disabled","placeholder"]),b("div",Re,[M(h(z),{type:"primary",size:"small",style:{width:"100%"},loading:p.value,onClick:re},{default:O(()=>[$(I(h(T)("mindmap.generateMindMapButton")),1)]),_:1},8,["loading"])]),b("div",Me,[b("h4",Ne,I(h(T)("mindmap.contentRequirements")),1),M(h(z),{text:"",onClick:oe},{default:O(()=>[$(I(h(T)("mindmap.tryDemoButton")),1)]),_:1})]),M(h(ce),{value:C.value,"onUpdate:value":v[1]||(v[1]=t=>C.value=t),type:"textarea",disabled:p.value,autosize:{minRows:8,maxRows:24},placeholder:"请用markdown语法输入您想要生成思维导图的内容或在上方使用描述让AI帮您完善！"},null,8,["value","disabled"])]),b("div",Le,[b("div",Ve,I(h(T)("mindmap.usageCredits")),1),b("div",null,[M(h(xe),{size:"small"},{default:O(()=>[M(h(z),{type:"primary",onClick:K},{default:O(()=>[$(I(h(T)("mindmap.exportHTML")),1)]),_:1}),M(h(z),{type:"primary",onClick:ae},{default:O(()=>[$(I(h(T)("mindmap.exportPNG")),1)]),_:1}),M(h(z),{type:"warning",onClick:V},{default:O(()=>[$(I(h(T)("mindmap.exportSVG")),1)]),_:1})]),_:1})])])]),b("div",Oe,[h(j)?de("",!0):(ee(),Z("header",je,[b("h2",Fe,I(h(T)("mindmap.title")),1)])),b("div",Ge,[(ee(),Z("svg",{ref_key:"svgRef",ref:N,class:"box-border w-full h-full border rounded-md"},null,512))])])]))}});export{Qe as default};
